...
describe("A SRV connection string", () => {
    describe("when correctly created", () => {
        it("should be valid", () => {
            expect(getConnectionString(SrvFormatOptions(
                "username",
                "password",
                "hostname",
                "db",
                "app"
            ))).toBe("mongodb+srv://username:password@hostname/?retryWrites=true&w=majority&appName=app");
        });
    });
});
...
describe("A simple connection string", () => {
    describe("when created with only the host and the database name", () => {
        it("should be valid", () => {
            expect(getConnectionString(SimpleFormatOptions(
                O.none,
                O.none,
                "hostname",
                O.none,
